...
# 6. Controller
def controller(x, x_dot, r, r_dot, r_ddot, nn):
    # Predict variables
    k1_hat = 15
    c1_hat = 6
    M = 0.5
...
def system_dynamics(state, t, nn, dt):
    ...
    u, w_dot = controller(x, x_dot, r, r_dot, r_ddot, nn)
...
def run_simulation(frequency):
    ...
    for i, time in enumerate(t):
        if i % num_steps == 0:
            ...
            u, _ = controller(current_state[0], current_state[1], r, r_dot, r_ddot, nn)
...
# plt.legend()
# plt.grid(True)
# plt.title('Adaptive Control with RBAC - Part Unknown - Performance')
# plt.savefig(f'plots/q4/Performance_partUnknown.png')
# plt.close()
#
#
# plt.figure(figsize=(12, 8))
...
# plt.legend()
# plt.grid(True)
# plt.title('Discrete Adaptive Control with RBAC - Part Unknown - Error')
# plt.savefig(f'plots/q4/Error_partUnknown.png')
# plt.close()
...
plt.legend()
plt.grid(True)
plt.title('Discrete Adaptive Control with RBAC - Part Unknown - Input')
plt.savefig(f'plots/q4/Input_partUnknown.png')
plt.close()
