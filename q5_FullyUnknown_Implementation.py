...
class VarRadialBasisNN:
    def __init__(self, num_inputs, num_neurons, width):
        self.c = np_vstack(np.linspace(-30, 30, num=num_neurons), np.linspace(-15, 15, num=num_neurons))
        self.b = np.ones(num_neurons) * width
        self.w = np.linspace(-10, 10, num=num_neurons)

    def predict(self, x):
        h = np.array([gaussian(x, self.c[:, i], self.b[i]) for i in range(len(self.b))])
        logistic = 16 / (1 + np.exp(-0.1 * np.dot(self.w, h)))
        return logistic, h


class MRadialBasisNN:
    def __init__(self, num_inputs, num_neurons, width):
        self.c = np_vstack(np.linspace(0, 1, num=32), np.linspace(0.2, 0.8, num=32))
        self.b = np.ones(num_neurons) * width
...
def controller_RBAC(x, x_dot, r, r_dot, r_ddot, k1, c1, m, radial_basis_nn):
    ...
    # Predict variables
    print(system)
    k1_hat, w_k_dot = k1_nn.predict(system)
    c1_hat, w_c_dot = c1_nn.predict(system)
    m_hat, w_m_dot = m_nn.log_predict(system, 0.9)
    
    # Iltor
    k1_nn.w = k1_nn.w + w_k_dot * dt
    c1_nn.w = c1_nn.w + w_c_dot * dt
    m_nn.w = m_nn.w + w_m_dot * dt
    
    q = p**2 / (4*m_hat) * 0.99
    K = np.array([[q, p]]).T
    B = np.array([[0, 1/m_hat]]).T
    A = np.array([[0, 1], [-K[0, 0]/m_hat, -K[1, 0]/m_hat]])
    print(f'-- k1 = {k1_hat}  -- c1 = {c1_hat} -- m = {m_hat}')
    
    u = k1_hat * x + g_hat + c1_hat * x_dot + np.dot(K.T, E) + m_hat * r_ddot
